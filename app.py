...
from vercel_kv import KV  # Importa o banco de dados da Vercel

app = Flask(__name__)
CORS(app)

# Conecta ao Vercel KV usando as variáveis de ambiente automáticas
kv = KV()

def load_ranking():
    # Busca a lista 'ranking' no Redis. Se não existir, retorna lista vazia.
    ranking = kv.get('ranking')
    return ranking if ranking else []

def save_ranking(data):
    # Salva a lista inteira no Redis
    kv.set('ranking', data)
...
def submit_score():
    ...
    save_ranking(ranking[:50]) # Salva apenas os top 50 para economizar espaço
